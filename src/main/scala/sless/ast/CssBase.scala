...
object CssBase {
  ...
  def merge(css1: CssBase, css2: CssBase): CssBase = {
    ...
    //Create rules according to unique selector list
    var mergedRuleList = Seq[RuleBase]()
    for (selector <- selectorOrderedList) {
      var thisSelectorRule = RuleBase(selector, Seq[DeclarationBase]()) //Temp rule to which mathcing declarations are added
      for (rule <- rulesList) {
